...
static const uint16_t dstmap_arrowzx[16]={ KEY_LEFT,KEY_RIGHT,KEY_UP,KEY_DOWN, KEY_Z,KEY_X,KEY_C,KEY_V, KEY_TAB,KEY_BACKSLASH,KEY_GRAVE,KEY_BACKSPACE, KEY_ENTER,KEY_SPACE,KEY_ESC };
...
static const uint16_t dstmap_wasdq[16]={ KEY_A,KEY_D,KEY_W,KEY_S, KEY_SPACE,KEY_Q,KEY_Q,KEY_J, KEY_N,KEY_COMMA,KEY_DOT,KEY_SLASH, KEY_ENTER,KEY_SPACE,KEY_ESC };
...
static const uint16_t dstmap_horzspace[16]={ KEY_LEFT,KEY_RIGHT,0,0, KEY_SPACE,KEY_X,KEY_UP,0, 0,0,0,0, 0,0,0,0 }; // For Beaver and the Land Left Behind: UpArrow is both swim and jump; space is only jump.
